...
void CliUI::renderDashboard(const StateManager& engine) {
    ...
    std::cout << " [TIER 2 CORE STATE]: " << stateTypeToString(coreMgr.GetCurrentStateType()) << " | Floor: Level "
              << engineCtx.currentFloorId << " | Target: "
              << (engineCtx.selectedDestinationId < 0 ? "None"
                                                      : "Room " + std::to_string(engineCtx.selectedDestinationId))
              << "\n";
...
void CliUI::renderGuidanceHUD(const EngineContext& context, std::size_t currentStep) {
    ...
    if (context.activeRoute.empty()) {
        ...
        std::cout << "| Step " << (currentStep + 1) << " of " << context.activeRoute.size() << " | Target: Room "
                  << context.selectedDestinationId << " |\n";
        for (std::size_t i = 0; i < context.activeRoute.size(); ++i) {
            const auto& wp = context.activeRoute[i];
            if (i == currentStep) {
                std::cout << "|  >>> [WAYPOINT " << (i + 1) << "] Node " << wp.nodeId << " (Floor " << wp.floorId
                          << ", Pos: " << wp.x << ", " << wp.y << ", " << wp.z << ") <-- YOU ARE HERE\n";
            }
            else {
                std::cout << "|      [WAYPOINT " << (i + 1) << "] Node " << wp.nodeId << " (Floor " << wp.floorId
                          << ", Pos: " << wp.x << ", " << wp.y << ", " << wp.z << ")\n";
